...
- **Load sting:** `audio/m-dot-load-preview.mp3` — ~**12.5s** excerpt from **Dreamscape (Prod. By Marco Polo)** (*egO anD The eneMy*, Bandcamp; cut from ~0:42 so the beat is obvious). Encoded **small for fast download** (~48 kb/s mono, 22.05 kHz; re-encode with `ffmpeg -i … -ac 1 -b:a 48k -ar 22050 out.mp3` if you replace the source). The `<audio>` tag uses **`preload="none"`** and a **`data-src`** (not `src`) so the MP3 does **not** compete with first paint. As soon as **first paint** fires, JS injects a **high-priority `rel=preload as=audio`** and starts playback — it does **not** wait for `window.load`, the hero image, idle time, or `canplaythrough`. **Skipped entirely** when the Network Information API reports **save-data**, **2g / slow-2g**, or **downlink &lt; 0.5 Mbps** (carousel starts immediately via **`m-dot-load-sting-end`**). First **`pointerdown`** starts playback immediately if autoplay was blocked (helps iOS). Plays at ~62% level once buffered. The **fade uses Web Audio `GainNode`** when supported (`exponentialRampToValueAtTime` over ~5s starting ~**7.35s** in), because **`HTMLMediaElement.volume` is often ignored on iOS** (audio stays loud then stops abruptly). Falls back to stepping `audio.volume` on older browsers. When the fade completes, it dispatches **`m-dot-load-sting-end`** so the carousel autoplay can start. The `data-src` may use a `?v=` query to bust caches after replacing the file. Re-cut with ffmpeg if you swap tracks (Bandcamp stream URLs expire; keep a file in `audio/`).
