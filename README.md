...
- **Per-slide crop:** Slideshow images use **`object-fit: cover`**. Override the focal point on any `.swiper-slide` with **`--m-dot-object-position`** (any CSS [`object-position`](https://developer.mozilla.org/en-US/docs/Web/CSS/object-position) value — `center`, `center top`, `50% 20%`, etc.). Omit it to keep the default **`center`**.
