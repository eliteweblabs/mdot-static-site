...
- **Load sting:** `audio/m-dot-load-preview.mp3` — ~4s excerpt from **Intro (Prod. By KAN)** (*egO anD The eneMy*, Bandcamp). On `load` (or first `pointerdown` if autoplay is blocked), it plays at ~55% volume, then **fades out over ~0.9s** starting ~2s in. Re-cut with ffmpeg if you swap tracks (Bandcamp stream URLs expire; keep a file in `audio/`).
