...
- **Load sting:** `audio/m-dot-load-preview.mp3` — ~7.5s excerpt from **Dreamscape (Prod. By Marco Polo)** (*egO anD The eneMy*, Bandcamp; cut from ~0:42 so the beat is obvious). Encoded **small for fast download** (~48 kb/s mono, 22.05 kHz; re-encode with `ffmpeg -i … -ac 1 -b:a 48k -ar 22050 out.mp3` if you replace the source). On `load` (or first `pointerdown` if autoplay is blocked), it plays at ~62% level. The **fade uses Web Audio `GainNode`** when supported (`exponentialRampToValueAtTime` over ~5s starting ~2.4s in), because **`HTMLMediaElement.volume` is often ignored on iOS** (audio stays loud then stops abruptly). Falls back to stepping `audio.volume` on older browsers. When the fade completes, it dispatches **`m-dot-load-sting-end`** so the carousel autoplay can start. The `<audio>` `src` may use a `?v=` query to bust caches after replacing the file. Re-cut with ffmpeg if you swap tracks (Bandcamp stream URLs expire; keep a file in `audio/`).
