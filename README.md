...
- **Per-slide crop:** Slideshow images use **`object-fit: cover`**. Override the focal point on any `.swiper-slide` with **`--m-dot-object-position`** (any CSS [`object-position`](https://developer.mozilla.org/en-US/docs/Web/CSS/object-position) value — `center center`, `top center`, `50% 20%`, etc.). **Slide 1** is **`top center`**; the other slides are **`center center`**. Omit the variable to keep the CSS default **`center`**.
